...
local military_ground_unit_names = {
  "European archer",
  "European crossbowman",
  "European spearman",
  "European pikeman",
...
local function ascension_extras()
  ...
  core.writeCodeInteger(0x400000 + 0x1B6360, 57) -- "Catapult base range.",           
  core.writeCodeInteger(0x400000 + 0x3597C, 3249) -- "Catapult control range 1.",      
  core.writeCodeInteger(0x400000 + 0x35EE9, 3249) -- "Catapult control range 2.",      
  core.writeCodeInteger(0x400000 + 0x36358, 3249) -- "Catapult control range 3.",      
  core.writeCodeInteger(0x400000 + 0x36728, 3249) -- "Catapult control range 4.",      
  core.writeCodeInteger(0x400000 + 0x368AA, 3249) -- "Catapult control range 5.",      
  core.writeCodeInteger(0x400000 + 0x369C8, 3249) -- "Catapult control range 6.",      
  core.writeCodeInteger(0x400000 + 0x36ACF, 3249) -- "Catapult control range 7.",      
...
namespace.apply_rebalance = function(config)
  ...
  local unit_gold_jumplist_addr = 0x4F6584
  ...
  if units ~= nil then
    ...
    for unit, stats in pairs(units) do
      ...
      if health ~= nil then
        address = unit_health_base + unit_idx * 4
        core.writeInteger(address, health)
      end

      if arrowDamage ~= nil then
        address = unit_arrow_dmg_base + unit_idx * 4
        core.writeInteger(address, arrowDamage)
      end

      if xbowDamage ~= nil then
        address = unit_xbow_dmg_base + unit_idx * 4
        core.writeInteger(address, xbowDamage)
      end
      ...
      if speedLevel ~= nil then
        address = unit_speed_base + unit_idx*4
        core.writeInteger(address, speedLevel)
      end
  ...
  if resources ~= nil then
    ...
    local hops_farmer_func = locate_aob("6A 01 74 04 6A 02 EB 02 6A 01 50 E8")  -- hops farm conditionally yields less goods (1 instead of 2)
    ...
    local custom_fletcher_code_addr = core.insertCode(fletcher_func, 9, custom_fletcher_code, fletcher_func+9, "after")
    ...
    local custom_poleturner_code_addr = core.insertCode(poleturner_func, 9, custom_poleturner_code, poleturner_func+9, "after")
    ...
    local custom_blacksmith_code_addr = core.insertCode(blacksmith_func, 9, custom_blacksmith_code, blacksmith_func+9, "after")
  ...
  if beer ~= nil then
    ...
    local coverage_addr = locate_aob("7F 05 33 C0 C2 04 00 8B 89 ? ? ? ? 85 C9 7E F1 69 C0 B8 0B 00 00 99")
    ...
    for key, val in pairs(beer) do
      ...
      if key == "coverage_per_inn" then
        core.writeCodeInteger(coverage_addr+19, val*100)
      end
    end
  end
  ...
  if taxation ~= nil then
    local tax_popularity_offset = 0x11F0BC0
    if data.version.isExtreme() then
    ...
    if tax_table ~= nil then
      ...
      local tax_table_addr = core.allocate(12*2)
